    ...
    teen: {
        ...
        steps: [
            ...
            { id: 3, title: "03. Pendidikan", fields: [
                { id: "namaSekolah", label: "Nama Sekolah", type: "text", placeholder: "SMA Negeri 1", validate: "min3" }
            ] },
            { id: 4, title: "04. Finansial", fields: [
                { id: "incomeSource", label: "Sumber Uang Saku", type: "select", options: ["-- Pilih --", "Uang Saku", "Hadiah", "Part Time", "Lainnya"], validate: "selected" }
            ] },
    ...
    adult: {
        ...
        steps: [
            ...
            { id: 2, title: "02. Kontak Utama", fields: [
                { id: "inputWA", label: "WhatsApp (Boleh Fiktif)", type: "tel", placeholder: "08xxxxxxxxxx", validate: "digit11-13" },
                { id: "inputEmail", label: "Email (Boleh Fiktif)", type: "email", placeholder: "email@kamu.com", validate: "email" }
            ] },
            ...
            { id: 5, title: "05. Pendapatan", fields: [
                { id: "income", label: "Penghasilan Bulanan", type: "number", placeholder: "Contoh: 10000000", validate: "digit" }
            ] },
            ...
            { id: 13, title: "13. Konfirmasi", fields: [
                { id: "checkAgree", label: "I confirm I agree to the Terms & Conditions", type: "checkbox", validate: "checked" }
            ] },
            ...
            { id: 15, title: "Finalisasi", isFinal: true }
        ]
    }
};
...
function setupInputListeners() {
    ...
    const nums = ['inputWA', 'inputNIK', 'inputPW', 'income', 'emPhone'];
    nums.forEach(id => {
...
function validateFields(stepIndex) {
    ...
    for(let field of stepConfig.fields) {
        ...
        if(field.validate === "digit") {
            if(!/^\d+$/.test(val)) { showModal("Format Salah", `${field.label} harus berupa angka!`); return false; }
        }
        ...
        if(field.validate === "selected") {
            if(val === "" || val.includes("-- Pilih")) { showModal("Pilihan Diperlukan", "Harap pilih salah satu opsi!"); return false; }
        }
...
async function startSecureValidation() {
    ...
    const countdown = setInterval(() => {
        ...
        else timerDisplay.innerText = `Finalisasi Penutuan Digital... (${timeLeft}s)`;
...
async function finalRevealProcess(stepDiv) {
    ...
    const fieldsToCheck = ['inputWA', 'inputEmail', 'jobType', 'income', 'incomeSource', 'accPurpose', 'waliName', 'emName', 'emPhone', 'address', 'secQuestion', 'emRelation', 'emPhone', 'address', 'secQuestion', 'secQuestion', 'emRelation', 'emPhone'];
    fieldsToCheck.forEach(id => {
        ...
        if (el && el.value && el.value.trim() !== "") {
            ...
            else if(id ===('emName') nasabahData.namaWali = el.value;
            else if(id ===('emPhone') nasabahData.noHpDarurat = el.value;
            else if(id ===('emRelation') nasabahData.hubungan = el.value;
            else if(id ===('emRelation') nasabahData.alamat = el.value;
            else if(id ===('emPhone') nasabahData.noHpDarurat = el.value;
            else if(id ===('address') nasabahData.alamat = el.value;
            else if(id ===('secQuestion') nasabahData.pertanyaan = el.value;
        }
    });
...
window.takeScreenshot = async (stepDiv) => {
    ...
    const cardInner = stepDiv.querySelector('.card-inner-ref');
    const nama = document.getElementById('inputNama').value;
